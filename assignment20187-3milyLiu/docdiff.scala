...
def occurrences(xs: List[String]): Map[String, Int] = {
    val getList = xs
    getList.groupBy(x => x) map{
        case(k,v)=>k->v.length
        //wrong way round but still works
        //try to do alphabetical
    }
}
...
def prod(lst1: List[String], lst2: List[String]) : Int = {
    val first = occurrences(lst1)
    val second = occurrences(lst2)
    val list = for((k,v)<-first
        if(second.contains(k))) yield
        v *second.getOrElse(k,0)
    val sum = list.sum
    sum
}
prod(list1,list2) // 7 
...
def overlap(lst1: List[String], lst2: List[String]) : Double = {
    val top = prod(lst1,lst2).toDouble
    val bottom = math.max(prod(lst1,lst1).toDouble, prod(lst2,lst2).toDouble)
    val total = top/bottom
    total
}

def similarity(s1: String, s2: String) : Double = {
    val cleans1 = clean(s1)
    val cleans2 = clean(s2)
    val sim = overlap(cleans1,cleans2)
    sim
}
