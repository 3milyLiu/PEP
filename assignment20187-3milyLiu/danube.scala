...
//     in the file).
//def drop(s:String) = s.drop(1)


def get_csv_url(url: String) : List[String] = {
    val get = scala.io.Source.fromURL(url).mkString
    val split =  get.split("\n")
    val drop = split.drop(1)
    drop.toList
    //Try(Source.fromURL(url).mkString.toList).
     }
...
def process_ratings(lines: List[String]) : List[(String, String)] = {
    val above4 = lines.filter(_.split(",")(2).toInt >= 4).toList
    val pairs = above4.map(x=>(x.split(",")(0),x.split(",")(1)))
    pairs
}

def process_movies(lines: List[String]) : List[(String, String)] = {
    val movies = lines.map(x=>(x.split(",")(0),x.split(",")(1)))
    movies
}
...
def suggestions(recs: Map[String, List[String]], 
                mov_name: String) : List[String] = {
                     favourites(recs, mov_name).flatten.groupBy(identity).mapValues(_.size).toList.sortBy(-_._2).map(_._1)
                }
...
def recommendations(recs: Map[String, List[String]],
                    ...
                    mov_name: String) : List[String] = {
                         for(i<-suggestions(recs, mov_name).take(2))yield
                         movs(i)
                    }
