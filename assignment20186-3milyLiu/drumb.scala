...
def get_january_data(symbol: String, year: Int) : List[String] = {
    val csv = Source.fromFile(symbol + ".csv").getLines().toList
    for(line<-csv
...
def get_first_price(symbol: String, year: Int) : Option[Double] = {
    val price = get_january_data(symbol, year)
    if(price.isEmpty){
        None
}
    else{
...
def get_prices(portfolio: List[String], years: Range) : List[List[Option[Double]]] = {
    val toList = years.toList
    val price = for(year <- toList) yield{
...
def get_delta(price_old: Option[Double], price_new: Option[Double]) : Option[Double] = {
    Try(Option((price_new.get-price_old.get)/price_old.get)).getOrElse(None)
    }
...
def get_deltas(data: List[List[Option[Double]]]) :  List[List[Option[Double]]] = {
    val all = for(year<-(1 to data.size - 1)) yield{
        val stock = data(year)
        (for(price<-(0 to stock.size - 1)) yield{
            get_delta(data(year - 1)(price), stock(price))}).toList
        }
    all.toList
}
...
def yearly_yield(data: List[List[Option[Double]]], balance: Long, index: Int) : Long = {
    val investment = data(index).flatten.size
    val earned = (for (value <- data(index).flatten) yield (value * balance/investment)).sum.toLong
    val total = balance + earned
    total
}
...
def compound_yield(data: List[List[Option[Double]]], balance: Long, index: Int) : Long = {
    //if(index == data.size - 1){
      if(index >= data.length){
        ...
        val calculate = yearly_yield(data, balance, index)
        compound_yield(data, calculate, index + 1)
    }
}
